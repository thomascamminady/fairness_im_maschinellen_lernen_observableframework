...
def make_normal_items(category: int, pays_back: bool, n: int, mean: float, std: float):
    ...
    for score in scores:
        ...
        total_density += pdf_value
        densities.append(pdf_value)
    
    # Calculate multiplier to match desired total
    multiplier = n / total_density
    
    # Second pass: create items with error accumulation
    error_accumulator = 0.0
    for score, density in zip(scores, densities):
        # Adjusted count including error from previous iterations
        e = error_accumulator + density * multiplier
        m = int(math.floor(e))
    ...
    # Add any remaining items to the peak of the distribution
    while len(items) < n:
        peak_score = scores[np.argmax(densities)]
        items.append({"category": category, "pays_back": pays_back, "score": peak_score})
...
def generate_js_style_distributions():
    ...
    df = (pl.DataFrame(comparison_items)
          ...
          .select(["score", "age", "pays_back", "type"]))
